...
## Automatic Water Tank Study

This simulation uses a simple automatic controller to keep a water tank between two levels:

- The pump turns on when the water level falls to `0.30 m`.
- The pump turns off when the water level rises to `0.75 m`.
- Water leaves the tank continuously at `0.008 m/s`.
- When the pump is on, it adds water at `0.025 m/s`.

The level changes during each small time step according to:

```text
level change = (pump flow - water usage) * time step
```

Using separate on and off levels is called hysteresis. It prevents the pump from switching rapidly on and off near a single control level.
